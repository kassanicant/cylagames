...
# AUDIO NODES
@onready var sfx_player = $"../SfxPlayer"       # Use this for clicks
@onready var music_player = $"../MusicPlayer"   # NEW: Make sure to add this node in your Scene!
...
func _ready():
	...
	# 1. LOAD AUDIO ASSETS (Using files from your upload)
	# Note: Loading .wav files as streams
	if ResourceLoader.exists("res://ASSETS/Background Music.wav"):
		audio_bg_music = load("res://ASSETS/Background Music.wav")
	
	if ResourceLoader.exists("res://ASSETS/Click(PenSound).wav"):
		audio_click = load("res://ASSETS/Click(PenSound).wav")

	# 2. START MUSIC
	if music_player and audio_bg_music:
		music_player.stream = audio_bg_music
		music_player.volume_db = -10 # Lower volume slightly so it's not too loud
		music_player.autoplay = true
		music_player.play()

	# 3. SETUP CLICK SFX
	if sfx_player and audio_click:
		sfx_player.stream = audio_click

	# 4. HIDE GAME OVER SCREEN
	if game_over_layer: game_over_layer.visible = false
	
	# 5. CONNECT BUTTONS (Adding sound to buttons too)
	if retry_btn: 
		retry_btn.pressed.connect(_on_retry_pressed)
		retry_btn.pressed.connect(_play_click_sound)
	if menu_btn: 
		menu_btn.pressed.connect(_on_menu_pressed)
		menu_btn.pressed.connect(_play_click_sound)

	# 6. LOAD VISUAL ASSETS
...
func _process(delta):
	...
	if not game_over and turn_timer and !turn_timer.is_stopped():
		...
		if time_left != last_timer_int:
			...
			if time_left <= 5 and time_left > 1:
...
func _play_click_sound():
	if sfx_player and audio_click:
		# Randomize pitch slightly for variety if you want
	...
	if box_owners.size() == grid_width * grid_height:
		...
		if Global.is_vs_bot:
			...
			if scores[0] > scores[1]:
				...
				title_label.modulate = color_p1
			elif scores[1] > scores[0]:
				title_label.text = "PLAYER 2 WINS!"
				title_label.modulate = color_p2
			else:
