...
var path_drop = "res://ASsets/BottlePutDown.wav"
var path_gameover = "res://ASsets/GameOver.wav"
...
class GameBox extends TextureRect:
	...
	func _drop_data(_at_pos, new_bottle):
		...
		if slot.get_child_count() > 0:
			var current = slot.get_child(0)
			slot.remove_child(current)
			old_parent.add_child(current)
...
func _ready():
	# 1. FIND NODES
	grid_container = find_child("BoxGrid", true, false)
	bench_slots = find_child("Slots", true, false)
	score_label = find_child("ScoreLabel", true, false)
	timer_label = find_child("TimerLabel", true, false)
	btn_back = find_child("BtnBack", true, false)
	game_over_panel = find_child("GameOverPanel", true, false)
	btn_reset = find_child("BtnReset", true, false)
	btn_next = find_child("BtnNext", true, false)
	...
	if btn_back: 
		btn_back.pressed.connect(play_click)
		btn_back.pressed.connect(func(): get_tree().change_scene_to_file("res://Scenes/LevelSelect.tscn"))
	if btn_reset: 
		btn_reset.pressed.connect(play_click)
		btn_reset.pressed.connect(start_level)
	if btn_next: 
		btn_next.pressed.connect(play_click)
...
func setup_audio():
	# Auto-create players if missing
	if not find_child("MusicPlayer", true, false):
	...
	if not find_child("SFXPlayer", true, false):
		sfx_player = AudioStreamPlayer.new()
		sfx_player.name = "SFXPlayer"
		add_child(sfx_player)
	else:
		sfx_player = find_child("SFXPlayer", true, false)

	# Play Music (LOWER VOLUME FOR GAMEPLAY)
	if music_player:
		music_player.stream = load(path_bg_music)
		music_player.volume_db = -20 # <--- LOWER VOLUME HERE
		music_player.play()

func start_level():
	if game_over_panel: game_over_panel.visible = false
	if music_player and not music_player.playing: music_player.play()
	
	# --- LEVEL CONFIGURATION ---
	var boxes_count = 3
	var dummies = 0
	
	if current_level == 1: boxes_count = 3; time_left = 15
	elif current_level == 2: boxes_count = 4; time_left = 20
	elif current_level == 3: boxes_count = 6; time_left = 30
	elif current_level == 4: boxes_count = 8; dummies = 2; time_left = 50
	elif current_level == 5: boxes_count = 10; dummies = 3; time_left = 60
	elif current_level == 6: boxes_count = 12; dummies = 4; time_left = 90
	else: boxes_count = 3; time_left = 60

	matches_needed = boxes_count
	update_ui(0)
	
	if grid_container: for c in grid_container.get_children(): c.queue_free()
	if bench_slots: for c in bench_slots.get_children(): c.queue_free()
	...
	if grid_container:
		for id in level_ids:
			...
			box.bottle_dropped.connect(play_drop)
...
func game_over(is_win):
	...
	if game_over_panel:
		...
		var lbl = game_over_panel.find_child("MessageLabel", true, false)
		if is_win:
			if lbl: lbl.text = "YOU WIN!"
			if btn_next: btn_next.visible = true
			if btn_reset: btn_reset.visible = true
			if grid_container: for c in grid_container.get_children(): c.queue_free()
			if bench_slots: for c in bench_slots.get_children(): c.queue_free()
		else:
			if lbl: lbl.text = "GAME OVER"
...
func play_click(): play_sfx(path_click)
func play_drop(): play_sfx(path_drop)
func play_sfx(path):
	if sfx_player:
